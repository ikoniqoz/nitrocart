...
class Checkout_dyn extends Checkout_core  
{
    ...
    public function shipping()
    {
        ...
        if($this->total_items_require_shipping == 0)
        {
            ...
            redirect( NC_ROUTE.'/checkout/review');   
        }
        ...
        if($this->input->post('selection') == 'new')
        {
            ...
            if ($this->form_validation->run())
            {
                ...
                if($this->validate_country_list_by_address_id( $address_id ))
                {
                    ...
                    $this->set_shipping_address_id( $address_id );
                    $this->set_step(3);//+
                    redirect(NC_ROUTE.'/checkout/shipment');//shipment is shipping options
                }
    ...
    public function shipment()
    {
        ...
        //lets see if we should be here
        $this->total_items_require_shipping > 0 OR  redirect( NC_ROUTE.'/checkout/billing');  
        ...
        switch(count($data->shipments))
        {
            ...
            case 1:
                $this->set_shipment_method($data->shipments[0]->id);
                $this->set_step(4);  
                redirect( NC_ROUTE.'/checkout/review');            
                break;
            default:
        ...
        $this->set_step(3);
        $this->template
                ->title(Settings::get('shop_name'), 'shipments')        
                ->set_breadcrumb('User Account',NC_ROUTE.'/checkout')
                ->set_breadcrumb('Billing',NC_ROUTE.'/checkout/billing')
                ->set_breadcrumb('Shipping',NC_ROUTE.'/checkout/shipping')
                ->set_breadcrumb('Shipping Options')
    ...
    public function review()
    {
        ...
        if( (! $this->show_review_order_page) AND ($this->is_step(4)) )
        {
            $this->set_step(5);               
            redirect(NC_ROUTE.'/checkout/gateway');
        }

        //downstep
        $this->is_step(4) OR redirect( NC_ROUTE.'/checkout/shipment'); 
        ...
        $data->order_total = nc_format_price( ($this->mycart->total() + $data->shipping_cost ) );
        ...
        $this->template
                ->title(Settings::get('shop_name'), 'Review')        
                ->set_breadcrumb('User Account',NC_ROUTE.'/checkout')
                ->set_breadcrumb('Billing',NC_ROUTE.'/checkout/billing')
                ->set_breadcrumb('Shipping',NC_ROUTE.'/checkout/shipping')
                ->set_breadcrumb('Shipping Options',NC_ROUTE.'/checkout/shipment')
                ->set_breadcrumb('Review Order')
    ...
    public function gateway()
    {
        ...
        //var_dump($input);die;
        //back down a step
        $this->is_step(5) OR redirect( NC_ROUTE.'/checkout/review'); 
        ...
        if ($this->form_validation->run())
        {
            ...
            //now place order
            if( $order_id = $this->place_order() > 0 )
            {
                redirect( NC_ROUTE.'/payment/order/'.$order_id);
            }
        ...
        switch($this->gateway_count)
        {
            case 0:
                //should not have this option, 
                //is handled in checkout_core
            case 1:
                $this->_processSingleGateway();
                break;
            default:
    ...
    /**
     * Since we only have 1 gateway we will force the 
     * user to this gateway to continue
    ...
    private function _displayMultipleGateways($data)
    {
        ...
        $this->template
                ->title( Settings::get('shop_name'), 'Gateway')
                ->set_breadcrumb('User Account',NC_ROUTE.'/checkout')
                ->set_breadcrumb('Billing',NC_ROUTE.'/checkout/billing')
                ->set_breadcrumb('Shipping',NC_ROUTE.'/checkout/shipping')
                ->set_breadcrumb('Shipping Options',NC_ROUTE.'/checkout/shipment');
                ...
                {
                    $this->template->set_breadcrumb('Review Order',NC_ROUTE.'/checkout/review');
                }
                $this->template
