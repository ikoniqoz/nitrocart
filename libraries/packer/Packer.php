...
class Packer
{
    ...
    protected $redir = 'nitrocart/cart'; 
     ...
     * redir_options
     */
    protected $redirect_options;


    /**
     * Constructor
     * redir_options are required
     * mode =admin|front
     */
    public function __construct($redir_options = ['redir'=>'nitrocart/cart', 'mode'=>'front'] ) {
      $this->items = new ItemList();
      $this->boxes = new BoxList();

      $this->redir = $redir_options['redir']; 

      $this->redirect_options = $redir_options;
    ...
    public function setItems($aItems) {
      if ($aItems instanceof ItemList) {
        ...
        $this->customException('Not a valid list of items');
      }
    }
     ...
     */
    public function pack($__redistribute_weight=true) {
      $packedBoxes = $this->doVolumePacking();

      //If we have multiple boxes, try and optimise/even-out weight distribution
      if($__redistribute_weight) {
      ...
      while ($this->items->count()) {
        ...
        if ($packedBoxesIteration->isEmpty()) {
          //throw new \RuntimeException('Item ' . $this->items->top()->getDescription() . ' is too large to fit into any box');
            ...
            if($this->redirect_options['mode']=='admin') 
            {
                $this->customException('Item ' . $this->items->top()->getAdminDescription() . ' is too large to fit into any box');
            }
            else
            {
                $this->customException('Item ' . $this->items->top()->getDescription() . ' is too large to fit into any box');
            }
    ...
    private function customException($message='Exception calculating packages',$err_type='error')
    {
        $this->session->set_flashdata($err_type,$message);
        redirect($this->redir);
    }
